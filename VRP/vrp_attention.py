...
class AttentionVRPActor(object):
    ...
    def __init__(self, dim, use_tanh=False, C=10, _name='Attention', _scope=''):
        ...
        self.project_query = tf.layers.Dense(dim, _scope=_scope+_name+'/proj_q')  # dense fully connected layer
        self.project_ref = tf.layers.Conv1D(dim, 1, _scope=_scope+_name+'/proj_ref') # conv1d_4
        self.C = C  # tanh exploration parameter, it's multiplied with the tanh
    ...
    def __call__(self, query, ref, env):
        ...
        demand = env.demand
        load = env.load
        max_time = tf.shape(demand)[1]  # 11 if we have 11 nodes, cause at max you  have to process 11 nodes?

        # embed demand and project it
        # emb_d:[batch_size x max_time x dim ]
        # perform conv1D
        emb_d = self.emb_d(tf.expand_dims(demand, 2))  # embedded demand
        # d:[batch_size x max_time x dim ]
        # apply a second conv1D
        d = self.project_d(emb_d)  # projected demand

        # embed load - demand
        # emb_ld:[batch_size*beam_width x max_time x hidden_dim]
        emb_ld = self.emb_ld(
            tf.expand_dims(
                tf.tile(tf.expand_dims(load, 1), [1, max_time]) - demand, 2))
...
# AttentionCritic works similarly to AttentionActor but with less parameters
# no load, no demands...
class AttentionVRPCritic(object):
    ...
    def __call__(self, query, ref, env):
        ...
        Args:
            ...
            env: keeps demand and load values and help decoding. Also it includes mask.
